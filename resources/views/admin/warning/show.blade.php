...
<main>
    ...
    <section class="p-5 rounded-sm flex flex-col flex-wrap items-start justify-start w-full gap-5 ">
        ...
        <div class=" flex flex-col items-start justify-start  my-2 mx-auto border  min-w-[50%] lg:py-0 mt-5" style=" box-shadow: 10px 10px 5px 0px rgba(70,107,94,0.75);
                ...
                <div class="my-3">
                    <a href="{{ route('admin.warning.index') }}" class="py-2 px-3  text-white rounded-md shadow-md bg-slate-400 w-full">Back</a>
                </div>
                <div class="flex items-start gap-2.5 w-full">
                    ...
                    <div class="flex items-center space-x-2 rtl:space-x-reverse">
                        <span class="font-semibold text-gray-900 dark:text-white">{{ $warning->name }}</span>
                        <span class="font-normal text-gray-500 dark:text-gray-400">{{ date('M d ,Y h:i a',strtotime($warning->created_at)) }}</span>
                    </div>
                    <div class="flex items-center space-x-2 rtl:space-x-reverse">
                        <span class="font-normal text-gray-500 dark:text-gray-400">{{ $warning->client->name }}</span>
                        <span class="font-normal text-gray-500 dark:text-gray-400">( {{ $warning->client->role }} )</span>
                    </div>
                    <div class="mb-5">
                        <span class="font-semibold text-2xl text-gray-900 dark:text-white">{{ $warning->name  }}</span>
                    </div>
                    <div class="mb-5">
                        <span class="font-semibold text-md italic text-gray-900 dark:text-white">{{ $warning->title  }}</span>
                    </div>
                    @foreach ($warning->message as $message)
